...
const read = async (req, res) => {
	...
	//fetching user
	const user = await models.User.fetchById(user_id, {
		withRelated: ["albums"],
	});
...
const readOne = async (req, res) => {
	...
	const photosAlbum = await models.Album.fetchById(album_id, {
		withRelated: ["photos"],
		columns: ["id", "title"],
	});
...
const register = async (req, res) => {
	...
	if (!errors.isEmpty()) {
		return res
			...
			.send({
				status: "fail",
				data: "the title needs to be at leat 3 characters long",
			});
	}
...
const registerPhoto = async (req, res) => {
	...
	const album = await models.Album.fetchById(album_id, {
		withRelated: ["photos"],
	});
	...
	if (!albumUser || !photosUser) {
		...
		try {
			...
			res.status(500).send({
				status: "error",
				message:
					"Exception thrown in database when adding a photo to an album.",
			});
			throw error;
		}
	}
	...
	if (!album) {
		...
		try {
			...
			res.status(500).send({
				status: "error",
				message:
					"Exception thrown in database when updating a new user.",
			});
			throw error;
		}
	}
